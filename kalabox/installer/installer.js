...
var install = flow('installKalabox')(
  ...
  function install8(exists) {
    ...
    // Uninstall Vagrant plugins in case we need to recompile
  function install9() {
    if (typeof VAGRANT_PLUGINS.length === 'undefined' || VAGRANT_PLUGINS.length < 1) {
      ...
      this.asyncEach(1)(VAGRANT_PLUGINS, function(plugin, group) {
        ...
        exec('vagrant plugin uninstall ' + plugin.name + ' --plugin-version ' + plugin.version, {cwd: KALASTACK_DIR}, group.async());
      });
    }
  },
  ...
  function install17(stdout, stderr) {
